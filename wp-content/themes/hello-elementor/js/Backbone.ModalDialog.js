...
Backbone.ModalView =
    Backbone.View.extend(
    {
        ...
        ensureModalBlanket:
            function()
            {
                this.modalBlanket = jQuery("#modal-blanket");

                if( this.modalBlanket.length == 0)
                {
                    this.modalBlanket =
                        jQuery("<div id='modal-blanket'>")
                            .css(
                                {
                                    position: "absolute",
                                    top: 0,
        ...
        setFocusOnFirstFormControl:
            function()
            {
                var controls = jQuery("input, select, email, url, number, range, date, month, week, time, datetime, datetime-local, search, color", jQuery(this.el));
                if( controls.length > 0)
                {
                    jQuery(controls[0]).focus();
                }
            },
        ...
        hideModal:
            function()
            {
                ...
                jQuery(document.body).unbind( "keyup", this.keyup);
                this.modalBlanket.unbind( "click", this.click);

                if( this.options.bodyOverflowHidden === true)
                {
                    jQuery(document.body).css( "overflow", this.originalBodyOverflowValue);
                }

                var container = this.modalContainer;
                jQuery(this.modalContainer)
                    .fadeOut(
        ...
        getCoordinate:
            function( coordinate, css)
            {
                if( typeof( css[coordinate]) !== "undefined")
                {
                    ...
                    delete css[coordinate]; // Don't apply positioning to the jQueryel, we apply it to the modal container. Remove it from options.css
		...
		recentre: // Re-centre the modal dialog after it has been displayed. Useful if the height changes after initial rendering eg; jquery ui tabs will hide tab sections
            function()
            {
                ...
                var centreY = jQuery(window).height() / 2;
                var centreX = jQuery(window).width() / 2;
                var modalContainer = this.modalContainer;
                var positionY = centreY  - (jQueryel.outerHeight() / 2);
                modalContainer.css({"top": (positionY + offsets.y) + "px"});

                var positionX = centreX - (jQueryel.outerWidth() / 2);
        ...
        getOffsets:
            function()
            {
                ...
                {
                    offsetY = jQuery(document).scrollTop(),
                    offsetX = jQuery(document).scrollLeft()
                }
        ...
        showModal:
            function( options)
            {
				...
				jQueryel.css( this.options.css);

                this.showModalBlanket();
                this.keyup = _.bind( this.keyup, this);
                this.click = _.bind( this.click, this);
                jQuery(document.body).keyup( this.keyup); // This handler is unbound in hideModal()
                this.modalBlanket.click( this.click); // This handler is unbound in hideModal()

                if( this.options.bodyOverflowHidden === true)
                {
                    this.originalBodyOverflowValue = jQuery(document.body).css( "overflow");
                ...
                if( this.options.showCloseButton)
                {
                    var view = this;
                    var image =
                        jQuery("<a href='#' id='modalCloseButton'>&#160;</a>")
                            .css({
									"position":"absolute",
									"top":"-10px",
									"right":"-10px",
                            ...
                            .hover(
                                function()
                                {
                                    jQuery(this).css( "background-image", "url(" + view.options.closeImageHoverUrl + ") !important");
                                },
                                function()
                                {
                                    jQuery(this).css( "background-image", "url(" + view.options.closeImageUrl + ") !important");
                                })
                            .click(
                                function( event)
                                {
